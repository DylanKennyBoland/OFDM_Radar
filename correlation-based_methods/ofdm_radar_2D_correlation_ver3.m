...
surface = surf(lagVec, nu, abs(X), 'FaceAlpha', 1);
surface.EdgeColor = "none";
title('\fontsize{14}\fontname{Georgia}Correlation-based Estimation');
labelX = xlabel('\fontname{Georgia}\bf Delay (Samples)');
labelY = ylabel('\fontname{Georgia}\bf\itf\rm\bf (Hz)');
labelZ = zlabel('\fontname{Georgia}\bf Magnitude of Correlation');
% Adjusting the axis label positions below so that the graph is more
% compact and neat:
labelX.Position(1) = 100;
labelX.Position(2) = -700;
labelY.Position(1) = -260;
labelY.Position(2) = 100;