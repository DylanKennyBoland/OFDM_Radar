%% OFDM 2D Correlation Radar Receiver with Modified Subcarriers (MSC):
% This is an experimentation script which investigates if there feasible
% results can be got by loading the subcarriers of the OFDM transmitter
% with the same symbols. 
% The benefits might prove to be:
% (1) Faster computation since each transmit symbol is made up of the same
% symbol.
% (2) Better control over Peak-to-Average-Power Ratio (PAPR) as the
% specific symbol chosen can be changed to suit needs.
% The 2D correlation formula is based on the ideas
% discussed in: "Comparison of Correlation-based OFDM Radar Receivers"
% Most variable names are based on those used in the above paper.
% Trying to compute equations 5 and 6 of the paper.
K = 64; % The number of subcarriers being used
M = 16; % The frame size
...
symbolNumber = 6; % We can easily change this to see if one symbol is more
...
for row = 2:K/2 % We start at row 2, as row 1 is empty for the DC subcarrier...
    Ftx(row, :) = signalSet(symbolNumber);
end
...
r = [zeros(delay, 1); dopplerShift.*s + Z]; % The received signal also has a delay in it
...
sm = s(1:L); % The signal sm. This can be defined outside the for loop below
% as sm is the same for all OFDM symbols or blocks (m)...
...
title("\fontsize{14}\fontname{Georgia}Modified Subcarrier (MSC) Correlation-based Estimation (\tau = " + delay + ", \itf_{D}\rm\bf = " + fD + " Hz, \sigma^{2}_{N} = " + noisedBW + " dBW, " + modOrder + "-" + modulationType + ", K = " + K + ", M = " + M + ", G = " + G + ")");
