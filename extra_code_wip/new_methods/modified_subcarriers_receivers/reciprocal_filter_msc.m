%% Reciprocal Filter with Modified Subcarriers (MSC):
% Investigating the effect of changing the symbols loaded onto the
% subcarriers. The script simulates the reciprocal filtering approach.
% More information can be found about the reciprocal filtering approach
% in "Comparison of Correlation-based OFDM Radar Receivers" and "Evaluation
% of the Ambiguity Function for Passive Radar with OFDM Transmitters"
K = 128; % The number of subcarriers being used
M = 16; % The frame size
...
% From the set of symbols we will choose a particular symbol to load onto
% each of the K subcarriers:
symbolNumber = 14; % We can easily change this to see if one symbol is more
% effective for radar than other

chosenSymbol = signalSet(symbolNumber)

Ftx = zeros(K/2, M); % Half of the empty transmit frame

Gain = 1; % The number by which each transmit symbol will be scaled... a higher
% gain corresponds to a greater transmit power...

for row = 2:K/2 % We start at row 2, as row 1 is empty for the DC subcarrier...
    % Each row (subcarrier) of the transmit frame is loaded with the same
    % symbol. And this is true for each column (OFDM symbol) too;
    Ftx(row, :) = Gain*signalSet(symbolNumber);
end
...
mean(abs(s)) % Let us see the mean absolute value of each symbol...

a = 9e-3; % How much the signal is attenuated by the target
L = K+cpLen; % The paper uses the extended block length (K + cyclic prefix length)
fD = 250; % The doppler shift caused by the target
delay = 20; % The delay of the received signal (r) in units of samples
p = 0:L*M-1; % The p vector, as discussed in the paper

% Computing the values in the received signal r:
noisedBW = -25;
Z = wgn(length(s), 1, noisedBW, 'complex'); % Complex Gaussian White noise matrix...
dopplerShift = exp(1i*2*pi*fD*p/L).';
r = [zeros(delay, 1); a*s.*dopplerShift + Z]; % The received signal; adding the delay...
...
% Since each subcarrier is loaded with the same complex symbol, we can work
% out sm once, outside the for loop below:
sm = 1./conj(s(1:L)); % The signal sm
sm(isinf(sm)) = 0;
...
title("\fontsize{14}\fontname{Georgia}Modified Subcarrier (MSC) Reciprocal Filter (\tau = " + delay + ", \itf_{D}\rm\bf = " + fD + " Hz, \sigma^{2}_{N} = " + noisedBW + " dBW, " + modOrder + "-" + modulationType + ", K = " + K + ", M = " + M + ", G = " + G + ")");
