...
% loaded into the Tx-side DFT block and the sequence of symbols that comes
% out the the Rx-side IDFT block.

% Setup of variables and system parameters:
N = 64; % no. of complex symbols going into Tx-side DFT block
M = 32; % no. of blocks or symbols being used in OFDM frame
...
% Channel parameters:
noisedBW = -100; % noise level in dBW
...
% Remove the CPs from each symbols:
r = r(cpLen+1:end, :); % removing the CPs from each symbol

% Now to map the relevant symbols to the i/p pins of the
% receive-side IDFT:
Frx = zeros(N, M); % received frame - currently empty

switch (mapType)
    case "interleave"
        % Now we need to map the outputs of the receive-side DFT block to
        % the inputs of the receive-side IDFT block which has (N) i/p pins:
        for i = 1:N
            Frx(i, :) = r(1+1+(i-1)*(interPadLength+1), :);
        end
    case "local"
        % insertIndex is the index where the first symbol from the output
        % of the Tx-side DFT block should be mapped to in the Tx-side IDFT
        ...
        % usually padded above and below by zeros:
        insertIndex = (excess + 1)/2 + 1 + 1;
        Frx = symbolMapOp(insertIndex:insertIndex+N-1, :);
    otherwise
        disp("ERROR: check the value of 'mapType' at the top of the script.")
        return
end

% Now we pass our received frame through the receive-side IDFT block:
Frx = ifft(Frx);

% Receiver-side Processing:
p = 0:N-1; % the vector p, as described in the paper referenced at the top of the program
nu = -800:25:800; % vector of frequencies

% The matrix below will store all the Xm matrices:
Xm_storage = zeros(M*length(nu), 2*N-1);
...
negDopplerShift = exp(-1i*2*pi*p_nu_matrix/N);
...
for block = 1:M
    ...
    sm = 1./conj(FtxCopy(:, block)); % the signal sm
    sm(isinf(sm)) = 0;
    rm = Frx(:, block); % the signal rm
    r_dash = rm.*negDopplerShift.';
    
    % Now to calculate the correlation:
    Xm = zeros(length(nu), 2*N-1);
...
Xm_dash = zeros(M*length(nu), 2*N-1);
...
delayVec = -N+1:N-1; % a vector of delays or lags (in units of samples)
...
title("\fontsize{14}\fontname{Georgia}Reciprocal Filter with DFT-spread (Delay = " + delay + ", f_{D} = " + fD + " Hz, Noise = " + noisedBW + " dBW, " + modOrder + "-" + modType + ", Mapping type = " + mapType + ")");
